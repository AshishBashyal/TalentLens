...
def test_enrich_jobs_with_kaggle_skills_prefers_structured_mapping(tmp_path: Path) -> None:
    with (tmp_path / "skills.csv").open("w", encoding="utf-8", newline="") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=["skill_abr", "skill_name"])
        writer.writeheader()
        writer.writerow({"skill_abr": "MRKT", "skill_name": "Marketing"})

    with (tmp_path / "job_skills.csv").open("w", encoding="utf-8", newline="") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=["job_id", "skill_abr"])
        writer.writeheader()
        writer.writerow({"job_id": "456", "skill_abr": "MRKT"})
    ...
    raw_job = RawJobPosting(
        ...
        experience="Entry level",
        employment_type="Full-time",
        skills="excellent writing. campaign analytics, communication",
    ...
    assert enriched[0].skills == "Marketing"
